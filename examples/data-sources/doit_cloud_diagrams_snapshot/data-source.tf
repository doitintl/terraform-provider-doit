...
locals {
  first_scheme = [for k, v in data.doit_cloud_diagrams_schemes.all.scheme : v][0]
...
data "doit_cloud_diagrams_snapshot" "example" {
  id          = local.first_layer.ssid
  snapshot_id = local.first_snapshot._id
}

output "snapshot_name" {
  value = data.doit_cloud_diagrams_snapshot.example.name
}

output "snapshot_created_at" {
  value = data.doit_cloud_diagrams_snapshot.example.created_at
}
