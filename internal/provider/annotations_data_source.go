...
func (d *annotationsDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	s := datasource_annotations.AnnotationsDataSourceSchema(ctx)
...
func (d *annotationsDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	if !data.SortBy.IsNull() {
		params.SortBy = new(models.ListAnnotationsParamsSortBy(data.SortBy.ValueString()))
	}
	if !data.SortOrder.IsNull() {
		params.SortOrder = new(models.ListAnnotationsParamsSortOrder(data.SortOrder.ValueString()))
	}
