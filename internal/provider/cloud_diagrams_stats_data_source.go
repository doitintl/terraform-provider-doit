...
import (
	...
	"github.com/doitintl/terraform-provider-doit/internal/provider/datasource_cloud_diagrams_stats"
...
var _ datasource.DataSource = (*cloudDiagramsStatsDataSource)(nil)
var _ datasource.DataSourceWithConfigure = (*cloudDiagramsStatsDataSource)(nil)

// NewCloudDiagramsStatsDataSource creates a new instance of the data source.
func NewCloudDiagramsStatsDataSource() datasource.DataSource {
	return &cloudDiagramsStatsDataSource{}
}

// cloudDiagramsStatsDataSource implements datasource.DataSource for cloud diagram stats.
type cloudDiagramsStatsDataSource struct {
	client *models.ClientWithResponses
}

// cloudDiagramsStatsDataSourceModel is the Terraform state model.
type cloudDiagramsStatsDataSourceModel struct {
...
func (d *cloudDiagramsStatsDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_cloud_diagrams_stats"
}

func (d *cloudDiagramsStatsDataSource) Schema(ctx context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	genSchema := datasource_cloud_diagrams_stats.CloudDiagramsStatsDataSourceSchema(ctx)
...
func (d *cloudDiagramsStatsDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *cloudDiagramsStatsDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data cloudDiagramsStatsDataSourceModel
	...
	if !req.Config.Raw.IsFullyKnown() {
		...
		data.CloudDiagramsStats = types.SetUnknown(datasource_cloud_diagrams_stats.CloudDiagramsStatsValue{}.Type(ctx))
...
func mapStatsToState(
	ctx context.Context,
	data *cloudDiagramsStatsDataSourceModel,
	stats []models.CloudDiagramStats,
) diag.Diagnostics {
	var diags diag.Diagnostics

	statsVals := make([]datasource_cloud_diagrams_stats.CloudDiagramsStatsValue, 0, len(stats))
	for _, s := range stats {
		// Map changes list.
		...
		val, valDiags := datasource_cloud_diagrams_stats.NewCloudDiagramsStatsValue(
			datasource_cloud_diagrams_stats.CloudDiagramsStatsValue{}.AttributeTypes(ctx),
	...
	statsList, listDiags := types.SetValueFrom(ctx, datasource_cloud_diagrams_stats.CloudDiagramsStatsValue{}.Type(ctx), statsVals)
	diags.Append(listDiags...)
	if diags.HasError() {
		return diags
	}
	data.CloudDiagramsStats = statsList
...
func mapStatsChanges(
	...
	var diags diag.Diagnostics
	elemType := datasource_cloud_diagrams_stats.ChangesValue{}.Type(ctx)

	if changes == nil || len(*changes) == 0 {
		emptyList, listDiags := types.ListValueFrom(ctx, elemType, []datasource_cloud_diagrams_stats.ChangesValue{})
		diags.Append(listDiags...)
		return emptyList, diags
	}

	vals := make([]datasource_cloud_diagrams_stats.ChangesValue, 0, len(*changes))
	for _, c := range *changes {
		...
		val, valDiags := datasource_cloud_diagrams_stats.NewChangesValue(
			datasource_cloud_diagrams_stats.ChangesValue{}.AttributeTypes(ctx),
			map[string]attr.Value{
				"type":    changeType,
...
func mapStatsImport(
...
) (datasource_cloud_diagrams_stats.ImportValue, diag.Diagnostics) {
	if importState == nil {
		return datasource_cloud_diagrams_stats.NewImportValueNull(), nil
	}
	...
	return datasource_cloud_diagrams_stats.NewImportValue(
		datasource_cloud_diagrams_stats.ImportValue{}.AttributeTypes(ctx),
		map[string]attr.Value{
