...
import (
	...
	"github.com/doitintl/terraform-provider-doit/internal/provider/datasource_cloud_diagram_schemes"
...
var _ datasource.DataSource = (*cloudDiagramSchemesDataSource)(nil)
var _ datasource.DataSourceWithConfigure = (*cloudDiagramSchemesDataSource)(nil)

// NewCloudDiagramSchemesDataSource creates a new instance of the data source.
func NewCloudDiagramSchemesDataSource() datasource.DataSource {
	return &cloudDiagramSchemesDataSource{}
}

// cloudDiagramSchemesDataSource implements datasource.DataSource for cloud diagram schemes.
type cloudDiagramSchemesDataSource struct {
	client *models.ClientWithResponses
}

// cloudDiagramSchemesDataSourceModel is the Terraform state model.
// It wraps the generated model and adds fields the generator cannot produce.
type cloudDiagramSchemesDataSourceModel struct {
	datasource_cloud_diagram_schemes.CloudDiagramSchemesModel
...
func (d *cloudDiagramSchemesDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_cloud_diagram_schemes"
}

func (d *cloudDiagramSchemesDataSource) Schema(ctx context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	genSchema := datasource_cloud_diagram_schemes.CloudDiagramSchemesDataSourceSchema(ctx)
...
func (d *cloudDiagramSchemesDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *cloudDiagramSchemesDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data cloudDiagramSchemesDataSourceModel
	...
	if !req.Config.Raw.IsFullyKnown() {
		...
		data.Scheme = types.MapUnknown(datasource_cloud_diagram_schemes.SchemeValue{}.Type(ctx))
		data.Statussheet = types.MapUnknown(datasource_cloud_diagram_schemes.StatussheetValue{}.Type(ctx))
		data.Template = datasource_cloud_diagram_schemes.NewTemplateValueUnknown()
...
func computeSchemesID(data cloudDiagramSchemesDataSourceModel) string {
	input := "cloud_diagram_schemes"
	if !data.SchemeIds.IsNull() {
...
func mapSchemesResponse(
	ctx context.Context,
	data *cloudDiagramSchemesDataSourceModel,
	...
	data.Template = datasource_cloud_diagram_schemes.NewTemplateValueNull()
...
func mapSchemeMap(
	...
	if scheme == nil || len(*scheme) == 0 {
		data.Scheme = types.MapNull(datasource_cloud_diagram_schemes.SchemeValue{}.Type(ctx))
		return diags
	}

	schemeVals := make(map[string]datasource_cloud_diagram_schemes.SchemeValue, len(*scheme))
	for key, sr := range *scheme {
		// Map statussheet list.
		ssVals := make([]datasource_cloud_diagram_schemes.SchemeStatussheetValue, 0, len(sr.Statussheet))
		for _, ss := range sr.Statussheet {
			ssVal, ssDiags := datasource_cloud_diagram_schemes.NewSchemeStatussheetValue(
				datasource_cloud_diagram_schemes.SchemeStatussheetValue{}.AttributeTypes(ctx),
		...
		ssList, listDiags := types.ListValueFrom(ctx,
			datasource_cloud_diagram_schemes.SchemeStatussheetValue{}.Type(ctx),
			ssVals,
		)
		...
		val, valDiags := datasource_cloud_diagram_schemes.NewSchemeValue(
			datasource_cloud_diagram_schemes.SchemeValue{}.AttributeTypes(ctx),
			map[string]attr.Value{
	...
	schemeMap, mapDiags := types.MapValueFrom(ctx,
		datasource_cloud_diagram_schemes.SchemeValue{}.Type(ctx),
		schemeVals,
	)
...
func mapStatussheetMap(
	...
	if statussheet == nil || len(*statussheet) == 0 {
		data.Statussheet = types.MapNull(datasource_cloud_diagram_schemes.StatussheetValue{}.Type(ctx))
		return diags
	}

	ssVals := make(map[string]datasource_cloud_diagram_schemes.StatussheetValue, len(*statussheet))
	...
	ssMap, mapDiags := types.MapValueFrom(ctx,
		datasource_cloud_diagram_schemes.StatussheetValue{}.Type(ctx),
		ssVals,
	)
	diags.Append(mapDiags...)
	if diags.HasError() {
...
func mapStatussheetEntry(
...
) (datasource_cloud_diagram_schemes.StatussheetValue, diag.Diagnostics) {
	var diags diag.Diagnostics

	attrVals := map[string]attr.Value{}

	// Map statussheet metadata.
	importVal := datasource_cloud_diagram_schemes.NewImportValueNull()
	if ssData.Statussheet.Import != nil {
		...
		importVal, importDiags = datasource_cloud_diagram_schemes.NewImportValue(
			datasource_cloud_diagram_schemes.ImportValue{}.AttributeTypes(ctx),
			map[string]attr.Value{
		...
		if diags.HasError() {
			return datasource_cloud_diagram_schemes.StatussheetValue{}, diags
		}
	}

	metaVal, metaDiags := datasource_cloud_diagram_schemes.NewStatussheetStatussheetValue(
		datasource_cloud_diagram_schemes.StatussheetStatussheetValue{}.AttributeTypes(ctx),
		map[string]attr.Value{
	...
	val, valDiags := datasource_cloud_diagram_schemes.NewStatussheetValue(
		datasource_cloud_diagram_schemes.StatussheetValue{}.AttributeTypes(ctx),
		attrVals,
	)
	diags.Append(valDiags...)
	return val, diags
...
func mapIssuesList(ctx context.Context, issues *[]models.CloudDiagramIssue) (basetypes.ListValue, diag.Diagnostics) {
	issueType := datasource_cloud_diagram_schemes.IssuesValue{}.Type(ctx)

	if issues == nil || len(*issues) == 0 {
		return types.ListValueFrom(ctx, issueType, []datasource_cloud_diagram_schemes.IssuesValue{})
	}

	var diags diag.Diagnostics
	vals := make([]datasource_cloud_diagram_schemes.IssuesValue, 0, len(*issues))
	for _, issue := range *issues {
		val, valDiags := datasource_cloud_diagram_schemes.NewIssuesValue(
			datasource_cloud_diagram_schemes.IssuesValue{}.AttributeTypes(ctx),
			map[string]attr.Value{
...
func mapSchemesPropsValue(ctx context.Context) datasource_cloud_diagram_schemes.PropsValue {
	...
	return datasource_cloud_diagram_schemes.NewPropsValueMust(
		datasource_cloud_diagram_schemes.PropsValue{}.AttributeTypes(ctx),
		map[string]attr.Value{},
	)
}
...
func mapNodeMap(ctx context.Context, nodes *map[string]models.CloudDiagramNode) (basetypes.MapValue, diag.Diagnostics) {
	nodeType := datasource_cloud_diagram_schemes.NodeValue{}.Type(ctx)
	...
	var diags diag.Diagnostics
	vals := make(map[string]datasource_cloud_diagram_schemes.NodeValue, len(*nodes))
	for key, n := range *nodes {
		...
		infraNode := datasource_cloud_diagram_schemes.NewInfraNodeValueNull()
		if n.InfraNode != nil {
			var infraDiags diag.Diagnostics
			infraNode, infraDiags = datasource_cloud_diagram_schemes.NewInfraNodeValue(
				datasource_cloud_diagram_schemes.InfraNodeValue{}.AttributeTypes(ctx),
				map[string]attr.Value{
		...
		val, valDiags := datasource_cloud_diagram_schemes.NewNodeValue(
			datasource_cloud_diagram_schemes.NodeValue{}.AttributeTypes(ctx),
			map[string]attr.Value{
...
func mapElementMap(ctx context.Context, elements *map[string]models.CloudDiagramElement) (basetypes.MapValue, diag.Diagnostics) {
	elemType := datasource_cloud_diagram_schemes.ElementValue{}.Type(ctx)
	...
	var diags diag.Diagnostics
	vals := make(map[string]datasource_cloud_diagram_schemes.ElementValue, len(*elements))
	for key, e := range *elements {
		...
		val, valDiags := datasource_cloud_diagram_schemes.NewElementValue(
			datasource_cloud_diagram_schemes.ElementValue{}.AttributeTypes(ctx),
			map[string]attr.Value{
...
func mapGroupMap(ctx context.Context, groups *map[string]models.CloudDiagramGroup) (basetypes.MapValue, diag.Diagnostics) {
	groupType := datasource_cloud_diagram_schemes.GroupValue{}.Type(ctx)
	...
	var diags diag.Diagnostics
	vals := make(map[string]datasource_cloud_diagram_schemes.GroupValue, len(*groups))
	for key, g := range *groups {
		...
		val, valDiags := datasource_cloud_diagram_schemes.NewGroupValue(
			datasource_cloud_diagram_schemes.GroupValue{}.AttributeTypes(ctx),
			map[string]attr.Value{
...
func mapGroupItemsList(ctx context.Context, items *[]models.CloudDiagramGroupItem) (basetypes.ListValue, diag.Diagnostics) {
	itemType := datasource_cloud_diagram_schemes.ItemsValue{}.Type(ctx)

	if items == nil || len(*items) == 0 {
		return types.ListValueFrom(ctx, itemType, []datasource_cloud_diagram_schemes.ItemsValue{})
	}

	var diags diag.Diagnostics
	vals := make([]datasource_cloud_diagram_schemes.ItemsValue, 0, len(*items))
	for _, item := range *items {
		val, valDiags := datasource_cloud_diagram_schemes.NewItemsValue(
			datasource_cloud_diagram_schemes.ItemsValue{}.AttributeTypes(ctx),
			map[string]attr.Value{
...
func mapLinkMap(ctx context.Context, links *map[string]models.CloudDiagramLink) (basetypes.MapValue, diag.Diagnostics) {
	linkType := datasource_cloud_diagram_schemes.LinkValue{}.Type(ctx)
	...
	var diags diag.Diagnostics
	vals := make(map[string]datasource_cloud_diagram_schemes.LinkValue, len(*links))
	for key, l := range *links {
		...
		val, valDiags := datasource_cloud_diagram_schemes.NewLinkValue(
			datasource_cloud_diagram_schemes.LinkValue{}.AttributeTypes(ctx),
			map[string]attr.Value{
...
func mapAttachmentMap(ctx context.Context, attachments *map[string]models.CloudDiagramAttachment) (basetypes.MapValue, diag.Diagnostics) {
	attType := datasource_cloud_diagram_schemes.AttachmentValue{}.Type(ctx)
	...
	var diags diag.Diagnostics
	vals := make(map[string]datasource_cloud_diagram_schemes.AttachmentValue, len(*attachments))
	for key, a := range *attachments {
		...
		val, valDiags := datasource_cloud_diagram_schemes.NewAttachmentValue(
			datasource_cloud_diagram_schemes.AttachmentValue{}.AttributeTypes(ctx),
			map[string]attr.Value{
...
func mapCombinerMap(ctx context.Context, combiners *map[string]models.CloudDiagramCombiner) (basetypes.MapValue, diag.Diagnostics) {
	cmbType := datasource_cloud_diagram_schemes.CombinerValue{}.Type(ctx)
	...
	var diags diag.Diagnostics
	vals := make(map[string]datasource_cloud_diagram_schemes.CombinerValue, len(*combiners))
	for key, c := range *combiners {
		...
		val, valDiags := datasource_cloud_diagram_schemes.NewCombinerValue(
			datasource_cloud_diagram_schemes.CombinerValue{}.AttributeTypes(ctx),
			map[string]attr.Value{
...
func mapNoteMap(ctx context.Context, notes *map[string]models.CloudDiagramNote) (basetypes.MapValue, diag.Diagnostics) {
	noteType := datasource_cloud_diagram_schemes.NoteValue{}.Type(ctx)
	...
	var diags diag.Diagnostics
	vals := make(map[string]datasource_cloud_diagram_schemes.NoteValue, len(*notes))
	for key, n := range *notes {
		val, valDiags := datasource_cloud_diagram_schemes.NewNoteValue(
			datasource_cloud_diagram_schemes.NoteValue{}.AttributeTypes(ctx),
			map[string]attr.Value{
				"_id":       types.StringValue(n.UnderscoreId),
				"color":     types.StringPointerValue(n.Color),
