...
func (ds *anomalyDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	if data.Id.IsUnknown() {
		...
		data.ProviderDisplayName = types.StringUnknown()
	...
	data.ActualCost = types.Float64PointerValue(nullableToPointer(anomaly.ActualCost))
	...
	data.ExpectedMaxCost = types.Float64PointerValue(nullableToPointer(anomaly.ExpectedMaxCost))
	data.MonitorLevel = types.StringValue(string(anomaly.MonitorLevel))
	data.Scope = types.StringValue(anomaly.Scope)
	data.EntityLabel = types.StringPointerValue(anomaly.EntityLabel)
	data.EntityName = types.StringPointerValue(anomaly.EntityName)
	data.ProviderDisplayName = types.StringPointerValue(anomaly.ProviderDisplayName)
