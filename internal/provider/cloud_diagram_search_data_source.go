...
import (
	...
	"github.com/doitintl/terraform-provider-doit/internal/provider/datasource_cloud_diagram_search"
...
var _ datasource.DataSource = (*cloudDiagramSearchDataSource)(nil)
var _ datasource.DataSourceWithConfigure = (*cloudDiagramSearchDataSource)(nil)

// NewCloudDiagramSearchDataSource creates a new instance of the data source.
func NewCloudDiagramSearchDataSource() datasource.DataSource {
	return &cloudDiagramSearchDataSource{}
}

// cloudDiagramSearchDataSource implements datasource.DataSource for cloud diagram search.
type cloudDiagramSearchDataSource struct {
	client *models.ClientWithResponses
}

// cloudDiagramSearchDataSourceModel is the Terraform state model.
type cloudDiagramSearchDataSourceModel struct {
...
func (d *cloudDiagramSearchDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_cloud_diagram_search"
}

func (d *cloudDiagramSearchDataSource) Schema(ctx context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	// Start from the generated schema (contains the computed output attributes).
	genSchema := datasource_cloud_diagram_search.CloudDiagramSearchDataSourceSchema(ctx)
...
func (d *cloudDiagramSearchDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *cloudDiagramSearchDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data cloudDiagramSearchDataSourceModel
	...
	if !req.Config.Raw.IsFullyKnown() {
		...
		data.Scheme = types.ListUnknown(datasource_cloud_diagram_search.SchemeValue{}.Type(ctx))
		data.Component = types.ListUnknown(datasource_cloud_diagram_search.ComponentValue{}.Type(ctx))
		data.Prop = types.ListUnknown(datasource_cloud_diagram_search.PropValue{}.Type(ctx))
...
func (d *cloudDiagramSearchDataSource) buildRequestBody(data cloudDiagramSearchDataSourceModel) models.SearchCloudDiagramsJSONRequestBody {
	body := models.SearchCloudDiagramsJSONRequestBody{
		Query: data.Query.ValueString(),
	}
...
func (d *cloudDiagramSearchDataSource) mapToState(
	ctx context.Context,
	data *cloudDiagramSearchDataSourceModel,
	...
	// Map scheme results.
	schemeVals := make([]datasource_cloud_diagram_search.SchemeValue, 0, len(schemes))
	for _, s := range schemes {
		val, valDiags := datasource_cloud_diagram_search.NewSchemeValue(
			datasource_cloud_diagram_search.SchemeValue{}.AttributeTypes(ctx),
	...
	schemeList, listDiags := types.ListValueFrom(ctx, datasource_cloud_diagram_search.SchemeValue{}.Type(ctx), schemeVals)
	...
	componentList, listDiags := types.ListValueFrom(ctx, datasource_cloud_diagram_search.ComponentValue{}.Type(ctx), componentVals)
	diags.Append(listDiags...)
	if diags.HasError() {
		return diags
	}
	...
	propList, listDiags := types.ListValueFrom(ctx, datasource_cloud_diagram_search.PropValue{}.Type(ctx), propVals)
	diags.Append(listDiags...)
	if diags.HasError() {
		return diags
	}
...
func mapPropsValue(ctx context.Context, apiProps *models.CloudDiagramComponentSearchItemProps) (datasource_cloud_diagram_search.PropsValue, diag.Diagnostics) {
	if apiProps == nil {
		return datasource_cloud_diagram_search.NewPropsValueNull(), nil
	}

	return datasource_cloud_diagram_search.NewPropsValue(
		datasource_cloud_diagram_search.PropsValue{}.AttributeTypes(ctx),
		map[string]attr.Value{
...
) ([]datasource_cloud_diagram_search.ComponentValue, diag.Diagnostics) {
	var diags diag.Diagnostics

	vals := make([]datasource_cloud_diagram_search.ComponentValue, 0, len(items))
	for _, c := range items {
		...
		val, valDiags := datasource_cloud_diagram_search.NewComponentValue(
			datasource_cloud_diagram_search.ComponentValue{}.AttributeTypes(ctx),
			map[string]attr.Value{
...
func mapPropSearchItems(
	ctx context.Context,
	items []models.CloudDiagramComponentSearchItem,
) ([]datasource_cloud_diagram_search.PropValue, diag.Diagnostics) {
	var diags diag.Diagnostics

	vals := make([]datasource_cloud_diagram_search.PropValue, 0, len(items))
	for _, c := range items {
		propsVal, propsDiags := mapPropsValue(ctx, c.Props)
		diags.Append(propsDiags...)
		if diags.HasError() {
		...
		val, valDiags := datasource_cloud_diagram_search.NewPropValue(
			datasource_cloud_diagram_search.PropValue{}.AttributeTypes(ctx),
			map[string]attr.Value{
				"_id":          types.StringValue(c.UnderscoreId),
				"account_name": types.StringPointerValue(c.AccountName),
				"color":        types.StringPointerValue(c.Color),
