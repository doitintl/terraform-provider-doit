...
import (
	...
	"github.com/doitintl/terraform-provider-doit/internal/provider/datasource_cloud_diagram_statussheet"
...
var _ datasource.DataSource = (*cloudDiagramStatussheetDataSource)(nil)
var _ datasource.DataSourceWithConfigure = (*cloudDiagramStatussheetDataSource)(nil)

// NewCloudDiagramStatussheetDataSource creates a new instance of the data source.
func NewCloudDiagramStatussheetDataSource() datasource.DataSource {
	return &cloudDiagramStatussheetDataSource{}
}

// cloudDiagramStatussheetDataSource implements datasource.DataSource.
type cloudDiagramStatussheetDataSource struct {
	client *models.ClientWithResponses
}

// cloudDiagramStatussheetDataSourceModel is the Terraform state model.
type cloudDiagramStatussheetDataSourceModel struct {
...
func (d *cloudDiagramStatussheetDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_cloud_diagram_statussheet"
}

func (d *cloudDiagramStatussheetDataSource) Schema(ctx context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	genSchema := datasource_cloud_diagram_statussheet.CloudDiagramStatussheetDataSourceSchema(ctx)
...
func (d *cloudDiagramStatussheetDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *cloudDiagramStatussheetDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data cloudDiagramStatussheetDataSourceModel
	...
	if !req.Config.Raw.IsFullyKnown() {
		data.Node = types.MapUnknown(datasource_cloud_diagram_statussheet.NodeValue{}.Type(ctx))
		data.Element = types.MapUnknown(datasource_cloud_diagram_statussheet.ElementValue{}.Type(ctx))
		data.Group = types.MapUnknown(datasource_cloud_diagram_statussheet.GroupValue{}.Type(ctx))
		data.Link = types.MapUnknown(datasource_cloud_diagram_statussheet.LinkValue{}.Type(ctx))
		data.Attachment = types.MapUnknown(datasource_cloud_diagram_statussheet.AttachmentValue{}.Type(ctx))
		data.Combiner = types.MapUnknown(datasource_cloud_diagram_statussheet.CombinerValue{}.Type(ctx))
		data.Note = types.MapUnknown(datasource_cloud_diagram_statussheet.NoteValue{}.Type(ctx))
...
func mapStatussheetComponentsToState(
	ctx context.Context,
	data *cloudDiagramStatussheetDataSourceModel,
...
func mapSSIssuesList(ctx context.Context, issues *[]models.CloudDiagramIssue) (basetypes.ListValue, diag.Diagnostics) {
	issueType := datasource_cloud_diagram_statussheet.IssuesValue{}.Type(ctx)

	if issues == nil || len(*issues) == 0 {
		return types.ListValueFrom(ctx, issueType, []datasource_cloud_diagram_statussheet.IssuesValue{})
	}

	var diags diag.Diagnostics
	vals := make([]datasource_cloud_diagram_statussheet.IssuesValue, 0, len(*issues))
	for _, issue := range *issues {
		val, valDiags := datasource_cloud_diagram_statussheet.NewIssuesValue(
			datasource_cloud_diagram_statussheet.IssuesValue{}.AttributeTypes(ctx),
...
func mapSSPropsValue(ctx context.Context) datasource_cloud_diagram_statussheet.PropsValue {
	return datasource_cloud_diagram_statussheet.NewPropsValueMust(
		datasource_cloud_diagram_statussheet.PropsValue{}.AttributeTypes(ctx),
		map[string]attr.Value{},
	)
}
...
func mapSSNodeMap(ctx context.Context, nodes *map[string]models.CloudDiagramNode) (basetypes.MapValue, diag.Diagnostics) {
	nodeType := datasource_cloud_diagram_statussheet.NodeValue{}.Type(ctx)
	...
	var diags diag.Diagnostics
	vals := make(map[string]datasource_cloud_diagram_statussheet.NodeValue, len(*nodes))
	for key, n := range *nodes {
		...
		infraNode := datasource_cloud_diagram_statussheet.NewInfraNodeValueNull()
		if n.InfraNode != nil {
			var infraDiags diag.Diagnostics
			infraNode, infraDiags = datasource_cloud_diagram_statussheet.NewInfraNodeValue(
				datasource_cloud_diagram_statussheet.InfraNodeValue{}.AttributeTypes(ctx),
				map[string]attr.Value{
		...
		val, valDiags := datasource_cloud_diagram_statussheet.NewNodeValue(
			datasource_cloud_diagram_statussheet.NodeValue{}.AttributeTypes(ctx),
			map[string]attr.Value{
...
func mapSSElementMap(ctx context.Context, elements *map[string]models.CloudDiagramElement) (basetypes.MapValue, diag.Diagnostics) {
	elemType := datasource_cloud_diagram_statussheet.ElementValue{}.Type(ctx)
	...
	var diags diag.Diagnostics
	vals := make(map[string]datasource_cloud_diagram_statussheet.ElementValue, len(*elements))
	for key, e := range *elements {
		...
		val, valDiags := datasource_cloud_diagram_statussheet.NewElementValue(
			datasource_cloud_diagram_statussheet.ElementValue{}.AttributeTypes(ctx),
			map[string]attr.Value{
...
func mapSSGroupMap(ctx context.Context, groups *map[string]models.CloudDiagramGroup) (basetypes.MapValue, diag.Diagnostics) {
	groupType := datasource_cloud_diagram_statussheet.GroupValue{}.Type(ctx)
	...
	var diags diag.Diagnostics
	vals := make(map[string]datasource_cloud_diagram_statussheet.GroupValue, len(*groups))
	for key, g := range *groups {
		...
		val, valDiags := datasource_cloud_diagram_statussheet.NewGroupValue(
			datasource_cloud_diagram_statussheet.GroupValue{}.AttributeTypes(ctx),
			map[string]attr.Value{
...
func mapSSGroupItemsList(ctx context.Context, items *[]models.CloudDiagramGroupItem) (basetypes.ListValue, diag.Diagnostics) {
	itemType := datasource_cloud_diagram_statussheet.ItemsValue{}.Type(ctx)

	if items == nil || len(*items) == 0 {
		return types.ListValueFrom(ctx, itemType, []datasource_cloud_diagram_statussheet.ItemsValue{})
	}

	var diags diag.Diagnostics
	vals := make([]datasource_cloud_diagram_statussheet.ItemsValue, 0, len(*items))
	for _, item := range *items {
		val, valDiags := datasource_cloud_diagram_statussheet.NewItemsValue(
			datasource_cloud_diagram_statussheet.ItemsValue{}.AttributeTypes(ctx),
			map[string]attr.Value{
...
func mapSSLinkMap(ctx context.Context, links *map[string]models.CloudDiagramLink) (basetypes.MapValue, diag.Diagnostics) {
	linkType := datasource_cloud_diagram_statussheet.LinkValue{}.Type(ctx)
	...
	var diags diag.Diagnostics
	vals := make(map[string]datasource_cloud_diagram_statussheet.LinkValue, len(*links))
	for key, l := range *links {
		...
		val, valDiags := datasource_cloud_diagram_statussheet.NewLinkValue(
			datasource_cloud_diagram_statussheet.LinkValue{}.AttributeTypes(ctx),
			map[string]attr.Value{
...
func mapSSAttachmentMap(ctx context.Context, attachments *map[string]models.CloudDiagramAttachment) (basetypes.MapValue, diag.Diagnostics) {
	attType := datasource_cloud_diagram_statussheet.AttachmentValue{}.Type(ctx)
	...
	var diags diag.Diagnostics
	vals := make(map[string]datasource_cloud_diagram_statussheet.AttachmentValue, len(*attachments))
	for key, a := range *attachments {
		...
		val, valDiags := datasource_cloud_diagram_statussheet.NewAttachmentValue(
			datasource_cloud_diagram_statussheet.AttachmentValue{}.AttributeTypes(ctx),
			map[string]attr.Value{
...
func mapSSCombinerMap(ctx context.Context, combiners *map[string]models.CloudDiagramCombiner) (basetypes.MapValue, diag.Diagnostics) {
	cmbType := datasource_cloud_diagram_statussheet.CombinerValue{}.Type(ctx)
	...
	var diags diag.Diagnostics
	vals := make(map[string]datasource_cloud_diagram_statussheet.CombinerValue, len(*combiners))
	for key, c := range *combiners {
		...
		val, valDiags := datasource_cloud_diagram_statussheet.NewCombinerValue(
			datasource_cloud_diagram_statussheet.CombinerValue{}.AttributeTypes(ctx),
			map[string]attr.Value{
...
func mapSSNoteMap(ctx context.Context, notes *map[string]models.CloudDiagramNote) (basetypes.MapValue, diag.Diagnostics) {
	noteType := datasource_cloud_diagram_statussheet.NoteValue{}.Type(ctx)
	...
	var diags diag.Diagnostics
	vals := make(map[string]datasource_cloud_diagram_statussheet.NoteValue, len(*notes))
	for key, n := range *notes {
		val, valDiags := datasource_cloud_diagram_statussheet.NewNoteValue(
			datasource_cloud_diagram_statussheet.NoteValue{}.AttributeTypes(ctx),
			map[string]attr.Value{
				"_id":       types.StringValue(n.UnderscoreId),
				"color":     types.StringPointerValue(n.Color),
