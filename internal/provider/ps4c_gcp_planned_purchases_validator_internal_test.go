...
func TestPs4cGcpPlannedPurchasesPageTokenValidator(t *testing.T) {
	t.Parallel()
	ctx := t.Context()
	...
	tests := []struct {
		...
		{
			name: "valid page_token with max_results",
			overrides: map[string]tftypes.Value{
				"page_token":  tftypes.NewValue(tftypes.String, "cursor-123"),
				"max_results": tftypes.NewValue(tftypes.Number, 1),
			},
			expectErr: false,
		},
		{
			name: "invalid page_token without max_results",
			overrides: map[string]tftypes.Value{
				"page_token": tftypes.NewValue(tftypes.String, "cursor-123"),
			},
			expectErr:   true,
		...
		{
			name: "deferred unknown page_token with null max_results",
			overrides: map[string]tftypes.Value{
		...
		{
			name: "deferred known page_token with unknown max_results",
			overrides: map[string]tftypes.Value{
				"page_token":  tftypes.NewValue(tftypes.String, "cursor-123"),
		...
		{
			name: "deferred both page_token and max_results unknown",
			overrides: map[string]tftypes.Value{
				"page_token":  tftypes.NewValue(tftypes.String, tftypes.UnknownValue),
				"max_results": tftypes.NewValue(tftypes.Number, tftypes.UnknownValue),
			},
		...
		{
			name: "deferred unknown page_token with known max_results",
			overrides: map[string]tftypes.Value{
				"page_token":  tftypes.NewValue(tftypes.String, tftypes.UnknownValue),
				"max_results": tftypes.NewValue(tftypes.Number, 1),
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			cfg := buildPs4cGcpPlannedPurchasesConfig(ctx, tt.overrides)
			validator := ps4cGcpPlannedPurchasesPageTokenValidator{}
			req := datasource.ValidateConfigRequest{Config: cfg}
			var resp datasource.ValidateConfigResponse
