...
func (r *datahubDatasetResource) Schema(ctx context.Context, _ resource.SchemaRequest, resp *resource.SchemaResponse) {
	...
	if attr, ok := s.Attributes["display_name"].(schema.StringAttribute); ok {
		attr.PlanModifiers = append(attr.PlanModifiers, useEmptyForUnknownWhenConfigNull())
		s.Attributes["display_name"] = attr
	}
...
func (r *datahubDatasetResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	overlayDatahubDatasetComputedFields(createResp.JSON201.Name, createResp.JSON201.Description, createResp.JSON201.DisplayName, createResp.JSON201.LogoName, nullableToPointer(createResp.JSON201.Records), createResp.JSON201.UpdatedBy, createResp.JSON201.LastUpdated, &plan)
...
func (r *datahubDatasetResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	overlayDatahubDatasetComputedFields(updateResp.JSON200.Name, updateResp.JSON200.Description, updateResp.JSON200.DisplayName, updateResp.JSON200.LogoName, nullableToPointer(updateResp.JSON200.Records), updateResp.JSON200.UpdatedBy, updateResp.JSON200.LastUpdated, &plan)
