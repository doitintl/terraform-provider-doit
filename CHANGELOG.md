...
- Upgraded Go to 1.27.0, adopted `httptest.NewTestServer` for in-memory mock testing, and flattened embedded struct composite literals
...
- Test-constructed `DCIRetryClient`s must now name their backoff policy, enforced by a new `retrypolicy` linter. Two call sites in the delete/not-found tests left it nil, which silently selected the production 2s-to-60s policy; since `Do` runs with no elapsed-time limit and both used an unbounded context, the first retryable status added to either table would have retried forever and surfaced as a package-wide test-timeout panic rather than a test failure. Those tests now inject a fast policy and bound their contexts
- Tests must now use `t.Context()` rather than `context.Background()`, enforced by `usetesting`'s `context-background` check. The check defaults to off, so the rule had been documented but unenforced. Two acceptance-test cleanup helpers keep an uncancellable context via `context.WithoutCancel(t.Context())`: they run from `t.Cleanup`, where `t.Context()` is already cancelled, and both only log warnings on failure — so converting them would have leaked test users and left shared tenant state mutated without any test failing
- A new `testserver` linter requires `httptest.NewTestServer` and rejects the 37 redundant `Close` calls that the older constructors made necessary. `NewTestServer` registers its own cleanup, so an explicit close is dead code — and inside a `testing/synctest` bubble it runs ahead of the response-body cleanups and can block on connections still in flight. `httptest.NewServer`, `NewTLSServer` and `NewUnstartedServer` are now rejected in tests too, which is what makes the redundancy rule sound rather than merely true today
