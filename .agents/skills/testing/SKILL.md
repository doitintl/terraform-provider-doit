...
3. **`NewTestServer` starts lazily.** Startup — and the assignment of `server.URL` — happens on the first `server.Client()` call. Read `server.URL` before anything has called `Client()` and you get `""`, silently. The common inline form `models.NewClientWithResponses(server.URL, models.WithHTTPClient(server.Client()))` is fine, because the method call is evaluated before the field read, but a test that reads `server.URL` without building a client from it must touch `server.Client()` first.
